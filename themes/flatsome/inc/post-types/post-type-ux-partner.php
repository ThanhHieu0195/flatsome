...
/* start post type */
if (!class_exists('Partner_Item_Post_Type')) :

	class Partner_Item_Post_Type
	{
		...
		public function __construct()
		{
			...
			// Add the partner_item post type and taxonomies
			add_action('init', array($this, 'partner_item_init'));

			// Thumbnail support for partner_item posts
			add_theme_support('post-thumbnails', array('partner_item'));

			// Add thumbnails to column view
			add_filter('manage_edit-partner_item_columns', array($this, 'add_thumbnail_column'), 10, 1);
			...
			// Show partner_item post counts in the dashboard
			add_action('right_now_content_table_end', array($this, 'add_partner_item_counts'));
		...
		/**
		 * Flushes rewrite rules on plugin activation to ensure partner_item posts don't 404.
		 *
		 * @link http://codex.wordpress.org/Function_Reference/flush_rewrite_rules
		 *
		 * @uses Featured Item_Post_Type::partner_item_init()
		 */
		public function plugin_activation()
		{
			$this->partner_item_init();
			flush_rewrite_rules();
		}
		 ...
		 */
		public function partner_item_init()
		{
		...
		{
			return array('partner_item_category', 'partner_item_tag');
		}
		...
		{
			$labels = array(
				'name'               => __('Partner', 'flatsome-admin'),
				'singular_name'      => __('Partner', 'flatsome-admin'),
				'add_new'            => __('Add New', 'flatsome-admin'),
				'add_new_item'       => __('Add New', 'flatsome-admin'),
				'edit_item'          => __('Edit Partner', 'flatsome-admin'),
				'new_item'           => __('Add New Partner', 'flatsome-admin'),
				'view_item'          => __('View Partner', 'flatsome-admin'),
				'search_items'       => __('Search Partner', 'flatsome-admin'),
			...
			$args = apply_filters('partner_itemposttype_args', $args);
		...
		protected function register_taxonomy_tag()
		{
			...
			$args = apply_filters('partner_itemposttype_tag_args', $args);

			register_taxonomy('partner_item_tag', array('partner_item'), $args);
		}
		...
		protected function register_taxonomy_category()
		{
			...
			$args = apply_filters('partner_itemposttype_category_args', $args);

			register_taxonomy('partner_item_category', array('partner_item'), $args);

			if (flatsome_option('partner_items_page')) {
				add_action('wp_loaded', 'add_ux_partner_item_permastructure');
				function add_ux_partner_item_permastructure()
				{
					$items_link = flatsome_option('partner_items_page');
					add_permastruct('partner_item_category',  $items_link . '/%partner_item_category%', false);
					add_permastruct('partner_item', $items_link . '/%partner_item_category%/%partner_item%', false);
				}

				add_filter('post_type_link', 'ux_partner_items_permalinks', 10, 2);
				function ux_partner_items_permalinks($permalink, $post)
				{
					if ($post->post_type !== 'partner_item')
						return $permalink;

					$terms = get_the_terms($post->ID, 'partner_item_category');

					if (!$terms)
						return str_replace('/%partner_item_category%', '', $permalink);

					$post_terms = array();
					foreach ($terms as $term)
						$post_terms[] = $term->slug;

					return str_replace('%partner_item_category%', implode(',', $post_terms), $permalink);
				}
		...
		/**
		 * Add taxonomy filters to the partner_item admin page.
		 *
		 ...
		 *
		 * @return null Return early if partner_item post type does not exist.
		 */
		public function add_partner_item_counts()
		{
			if (!post_type_exists('partner_item')) {
				return;
			}

			$num_posts = wp_count_posts('partner_item');

			// Published items
			$href = 'edit.php?post_type=partner_item';
			...
			// Pending items
			$href = 'edit.php?post_status=pending&amp;post_type=partner_item';
...
?>
			<tr>
				<td class="first b b-partner_item"><?php echo $number; ?></td>
				<td class="t partner_item"><?php echo $label; ?></td>
			</tr>
<?php
		}
	}

	new Partner_Item_Post_Type;
