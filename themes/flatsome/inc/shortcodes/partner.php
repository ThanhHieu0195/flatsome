...
// [partner_items_slider]
function flatsome_partner_shortcode($atts, $content = null, $tag)
{
  ...
  if (!$style) $style = flatsome_option('partner_style');

  // Fix old
  if ($tag == 'partner_items_slider') $type = 'slider';

  // Fix order
  if ($orderby == 'menu_order') $order = 'asc';

  // Set Classes
  $classes_box = array('partner-box', 'box', 'has-hover');
  $classes_image = array();
  ...
  echo '<div id="' . $_id . '" class="partner-element-wrapper has-filtering">';
...
?>
    <div class="container mb-half">
      <ul class="nav nav-<?php echo $filter; ?> nav-<?php echo $filter_align; ?> nav-<?php echo $filter_nav; ?> nav-uppercase filter-nav">
        ...
        <?php
        $tax_terms = get_terms('partner_item_category');
  ...
  if (isset($atts['ids'])) {
    ...
    if (!empty($atts['cat'])) {
      $args['tax_query'] = array(
        array(
          'taxonomy' => 'partner_item_category',
          'field' => 'id',
          'terms' => $cat,
        ),
      );
  ...
  ?>
      <div class="<?php echo implode(' ', $classes_col); ?>" data-id="<?php echo strip_tags(get_the_term_list(get_the_ID(), 'partner_item_category', "", ", ")); ?>" <?php echo $animate; ?>>
            ...
            <div class="<?php echo implode(' ', $classes_box); ?>">
              ...
              <div class="<?php echo implode(' ', $classes_text); ?>" <?php echo get_shortcode_inline_css($css_args); ?>>
                <div class="box-text-inner">
                  <h6 class="uppercase partner-box-title"><?php the_title(); ?></h6>
                  <p class="uppercase partner-box-category is-xsmall op-6">
                    <span class="show-on-hover">
                      <?php echo strip_tags(get_the_term_list(get_the_ID(), 'partner_item_category', "", ", ")); ?>
                    </span>
                  </p>
                </div>
              </div>
...
add_shortcode("ux_partner", "flatsome_partner_shortcode");
